...
from test_fixtures import reference_language as language
from test_fixtures import reference_society as society
...
class Lonk:
    ...
    def __init__(self, world, name=None):
        ...
        self.linguistics = language.initial_language()
    ...
    def speak(self, message=None, target=None, context='general', **values):
        if not self.world.toggles['talk']:
            return
        generated = False
        if message is None and context == 'general' and self.world.rng.random() < self.world.config['culture']['learned_speech_chance']:
            message = language.compose(self)
            generated = message is not None
        message = message or self.world.line(context, **values)
        if not generated:
            self.linguistics['total_spoken'] = min(1000000, self.linguistics['total_spoken'] + 1)
    ...
    def respond_to_player(self, message):
        ...
        learned = language.compose(self, prompt=message)
        if self.crush == 'Player':
            reply = self.world.line('crush') + (' ' + learned if learned else '')
        elif 'hug' in words:
...
class LonkWorld:
    ...
    def to_dict(self):
        return {'schema': SCHEMA, 'tick': self.tick, 'max_population': self.max_population,
                ...
                'colonies': copy.deepcopy(self.colonies), 'chronicle': copy.deepcopy(self.chronicle),
    ...
    def load(cls, path, controller=None):
        try:
            ...
            for record in data['lonks']:
                ...
                lonk.world = world
                if legacy:
                    lonk.linguistics = language.initial_language()
            ...
            if legacy:
                # Learn only from retained speech; preserve the old RNG stream.
