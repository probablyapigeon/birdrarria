...
class BridgeState:
    ...
    def handle(self, message: dict[str, Any]) -> dict[str, Any]:
        ...
        if kind == "hello":
            return {"ok": True, "version": PROTOCOL_VERSION, "capabilities": ["observe", "visit", "teach", "project", "bounded_build_requests"], "birds": sorted(BIRDS)}
        if kind in {"observe", "teach"}:
            text = payload["text"]
            entry = {"bird": bird, "world": world, "kind": kind, "text": text[:MAX_TEXT]}
            with self.lock:
                for member in BIRDS:
                    self.memories[member].append(entry)
            return {"ok": True, "recorded": entry, "shared": self.snapshot()}
        if kind == "visit":
            return {"ok": True, "visitor": bird, "world": world, "shared": self.snapshot()}
        if kind == "project":
            action = payload.get("action", "status")
            with self.lock:
                if action == "start":
                    self.project["name"] = payload.get("name", self.project["name"])
                    self.project["goal"] = payload.get("goal", self.project["goal"])
                elif action == "progress":
                    note = payload.get("note", "")
                    if isinstance(note, str) and note.strip():
                        self.project["progress"].append({"bird": bird, "world": world, "note": note[:MAX_TEXT]})
        ...
        if kind == "build_request":
            ...
            with self.lock:
                ...
                if action == "propose":
                    request = {"id": request_id, "status": "pending", "bird": bird, "world": world, "target": payload["target"], "bounds": payload["bounds"]}
                    self.builds.append(request)
                else:
                    request = next((item for item in reversed(self.builds) if item["id"] == request_id), None)
                    if request is None:
                        return {"ok": False, "error": "unknown build request"}
                    request["status"] = "approved" if action == "approve" else "rejected"
                    request["decided_by"] = bird
...
class Handler(socketserver.StreamRequestHandler):
    def handle(self) -> None:
        for raw in self.rfile:
            try:
                message = decode(raw.rstrip(b"\r\n"))
                response = self.server.state.handle(message)  # type: ignore[attr-defined]
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Loopback Lonk bird portal bridge")
    parser.add_argument("--port", type=int, default=45871)
    args = parser.parse_args()
    server = BridgeServer(args.port)
    print(f"Lonk portal bridge listening on 127.0.0.1:{args.port}")
    try:
